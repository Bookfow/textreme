    ...
    <div className="h-full flex flex-col" style={{ backgroundColor: themeStyle.pageBg, fontFamily: "'Noto Sans KR', system-ui, sans-serif" }}>
      ...
      {showSettings && (<><div className="fixed inset-0 z-[55]" onClick={() => setShowSettings(false)} /><div className="fixed z-[56] overflow-y-auto" style={{ bottom: 12, left: '50%', transform: 'translateX(-50%)', width: 'calc(100% - 32px)', maxWidth: 420, maxHeight: '75vh', borderRadius: 20, backgroundColor: theme === 'dark' ? 'rgba(30,26,22,0.78)' : theme === 'sepia' ? 'rgba(248,241,227,0.78)' : 'rgba(255,255,255,0.78)', backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', border: `1px solid ${themeStyle.border}`, boxShadow: '0 8px 40px rgba(0,0,0,0.35)' }}>
        ...
        <div style={{ padding: '8px 28px 36px', display: 'flex', flexDirection: 'column', gap: 28 }}>
          ...
          {/* 글꼴 */}
          <div><p style={{ fontSize: 12, fontWeight: 500, color: themeStyle.muted, marginBottom: 14 }}>글꼴</p><div style={{ display: 'flex', gap: 10 }}>{(Object.keys(FONTS) as ReflowFont[]).map(f => (<button key={f} onClick={() => setFont(f)} style={{ flex: 1, padding: '10px 0', borderRadius: 12, fontSize: 14, fontFamily: FONTS[f].family, border: `1.5px solid ${font === f ? ACCENT : themeStyle.border}`, backgroundColor: font === f ? `${ACCENT}12` : 'transparent', color: font === f ? ACCENT : themeStyle.text, cursor: 'pointer', fontWeight: font === f ? 600 : 400 }}>{FONTS[f].label}</button>))}</div></div>
          {/* 글자 크기 */}
