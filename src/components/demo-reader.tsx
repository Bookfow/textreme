  ...
  return (
    <div className="h-full flex flex-col" style={{ backgroundColor: themeStyle.pageBg, fontFamily: "'Noto Sans KR', system-ui, sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 12, padding: "14px 24px", width: "100%", maxWidth: 520 }}>
        ...
        <button onClick={e => { e.stopPropagation(); setShowHighlightPanel(!showHighlightPanel) }} className="flex flex-col items-center justify-center py-2.5 rounded-lg" style={{ color: showHighlightPanel ? ACCENT : highlights.length > 0 ? ACCENT : themeStyle.muted }}><Highlighter className="w-4 h-4" /><span style={{ fontSize: 10, marginTop: 6, opacity: 0.9 }}>형광펜</span></button>
        <button onClick={e => { e.stopPropagation(); toggleBM() }} onContextMenu={e => { e.preventDefault(); e.stopPropagation(); setShowBookmarkPanel(!showBookmarkPanel) }} className="flex flex-col items-center justify-center py-2.5 rounded-lg hover:opacity-70" style={{ color: isCurrentPageBookmarked ? ACCENT : themeStyle.muted }}>{isCurrentPageBookmarked ? <BookmarkCheck className="w-4 h-4" /> : <Bookmark className="w-4 h-4" />}<span style={{ fontSize: 10, marginTop: 6, opacity: 0.9 }}>책갈피</span></button>
