...
.epub-focus-active .epub-content [data-epub-adjacent="true"]{opacity:0.25!important}
  ...
  // 키보드
  useEffect(() => { const h = (e: KeyboardEvent) => { const t = (e.target as HTMLElement)?.tagName; if (t === 'INPUT' || t === 'TEXTAREA') return; if (showSettings || showToc || showNotesPanel || showNotesPanel || showSearch || showPaywall) return; if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') { e.preventDefault(); goPrev() } else if (e.key === 'ArrowRight' || e.key === 'ArrowDown' || e.key === ' ') { e.preventDefault(); goNext() } else if ((e.key === 'f' || e.key === 'F') && (e.ctrlKey || e.metaKey)) { e.preventDefault(); setShowSearch(p => !p) } }; window.addEventListener('keydown', h); return () => window.removeEventListener('keydown', h) }, [goNext, goPrev, showSettings, showToc, showNotesPanel, showNotesPanel, showSearch, showPaywall])
  ...
  return (
    <div className="h-full flex flex-col" style={{ backgroundColor: themeStyle.pageBg, fontFamily: "'Noto Sans KR', system-ui, sans-serif" }}>
      ...
      {/* 상단 바 */}
      <div style={{ borderColor: themeStyle.border, display: "flex", justifyContent: "center", borderBottom: `1px solid ${themeStyle.border}`, flexShrink: 0 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 12, padding: "14px 24px", width: "100%", maxWidth: 520 }}>
        <button onClick={e => { e.stopPropagation(); setShowToc(!showToc) }} className="flex flex-col items-center justify-center py-2.5 rounded-lg hover:opacity-70" style={{ color: showToc ? ACCENT : themeStyle.muted }}><List className="w-4 h-4" /><span style={{ fontSize: 10, marginTop: 6, opacity: 0.9 }}>목차</span></button>
        <button onClick={e => { e.stopPropagation(); setShowSearch(!showSearch) }} className="flex flex-col items-center justify-center py-2.5 rounded-lg hover:opacity-70" style={{ color: showSearch ? ACCENT : themeStyle.muted }}><Search className="w-4 h-4" /><span style={{ fontSize: 10, marginTop: 6, opacity: 0.9 }}>검색</span></button>
        <button onClick={e => { e.stopPropagation(); setFocusMode(!focusMode) }} className="flex flex-col items-center justify-center py-2.5 rounded-lg" style={{ color: focusMode ? ACCENT : themeStyle.muted, backgroundColor: focusMode ? `${ACCENT}15` : 'transparent' }}><Focus className="w-4 h-4" /><span style={{ fontSize: 10, marginTop: 6, opacity: 0.9 }}>집중</span></button>
      ...
      {showSettings && (<><div className="fixed inset-0 z-[55]" onClick={() => setShowSettings(false)} /><div className="fixed bottom-0 left-1/2 -translate-x-1/2 z-[56] rounded-t-2xl shadow-2xl max-h-[70vh] overflow-y-auto w-full max-w-lg" style={{ backgroundColor: theme === 'dark' ? 'rgba(36,30,24,0.85)' : theme === 'sepia' ? 'rgba(248,241,227,0.85)' : 'rgba(255,255,255,0.85)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', borderTop: `1px solid ${themeStyle.border}`, marginBottom: 8 }}><div className="flex justify-center pt-4 pb-2"><div className="w-10 h-1 rounded-full" style={{ backgroundColor: themeStyle.border }} /></div><div style={{ padding: '4px 28px 32px', display: 'flex', flexDirection: 'column', gap: 28 }}>
        ...
        <div className="grid grid-cols-2 gap-6"><div><div className="flex items-center justify-between mb-2"><p className="text-[10px] font-medium" style={{ color: themeStyle.muted }}>여백</p><span className="text-[10px] font-mono" style={{ color: themeStyle.text }}>{marginSize}px</span></div><input type="range" min={8} max={80} step={4} value={marginSize} onChange={e => setMarginSize(Number(e.target.value))} className="w-full h-1.5 rounded-full appearance-none cursor-pointer accent-amber-500" style={{ backgroundColor: themeStyle.border }} /></div><div><div className="flex items-center justify-between mb-2"><p className="text-[10px] font-medium" style={{ color: themeStyle.muted }}>자간</p><span className="text-[10px] font-mono" style={{ color: themeStyle.text }}>{(letterSpacing * 0.5).toFixed(1)}px</span></div><input type="range" min={-2} max={4} step={0.5} value={letterSpacing} onChange={e => setLetterSpacing(Number(e.target.value))} className="w-full h-1.5 rounded-full appearance-none cursor-pointer accent-amber-500" style={{ backgroundColor: themeStyle.border }} /></div></div>
        <div><p className="text-xs font-medium mb-4" style={{ color: themeStyle.muted }}>정렬</p><div className="flex gap-2">{(['left', 'justify'] as ReflowAlign[]).map(a => (<button key={a} onClick={() => setTextAlign(a)} className={`flex-1 py-2.5 rounded-xl text-xs flex items-center justify-center gap-2 border ${textAlign === a ? 'border-amber-500' : ''}`} style={{ backgroundColor: textAlign === a ? `${ACCENT}15` : 'transparent', borderColor: textAlign === a ? ACCENT : themeStyle.border, color: textAlign === a ? ACCENT : themeStyle.text }}>{a === 'left' ? <><AlignLeft className="w-4 h-4" /> 왼쪽</> : <><AlignJustify className="w-4 h-4" /> 양쪽</>}</button>))}</div></div>
      </div></div></>)}
      ...
      {/* 하단 프로그레스 바 */}
      {internalChapters.length > 0 && (<div style={{ borderTop: `1px solid ${themeStyle.border}`, backgroundColor: themeStyle.bg, flexShrink: 0, width: "100%", display: "flex", justifyContent: "center" }}>
