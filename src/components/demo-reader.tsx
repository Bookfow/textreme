...
.epub-focus-active .epub-content [data-epub-adjacent="true"]{opacity:0.25!important}
  ...
  return (
    <div className="h-full flex flex-col" style={{ backgroundColor: themeStyle.pageBg, fontFamily: "'Noto Sans KR', system-ui, sans-serif" }}>
      ...
      <div className="grid grid-cols-7 px-1 py-3 w-full" style={{ maxWidth: 420 }}>
        <button onClick={() => setShowToc(!showToc)} className="flex flex-col items-center justify-center py-2 rounded-lg hover:opacity-70" style={{ color: showToc ? ACCENT : themeStyle.muted }}><List className="w-4 h-4" /><span className="text-[9px] mt-1">목차</span></button>
        <button onClick={() => setShowSearch(!showSearch)} className="flex flex-col items-center justify-center py-2 rounded-lg hover:opacity-70" style={{ color: showSearch ? ACCENT : themeStyle.muted }}><Search className="w-4 h-4" /><span className="text-[9px] mt-1">검색</span></button>
        <button onClick={() => setFocusMode(!focusMode)} className="flex flex-col items-center justify-center py-2 rounded-lg" style={{ color: focusMode ? ACCENT : themeStyle.muted, backgroundColor: focusMode ? `${ACCENT}15` : 'transparent' }}><Focus className="w-4 h-4" /><span className="text-[9px] mt-1">집중</span></button>
        ...
        <button onClick={() => setShowHighlightPanel(!showHighlightPanel)} className="flex flex-col items-center justify-center py-2 rounded-lg" style={{ color: showHighlightPanel ? ACCENT : highlights.length > 0 ? ACCENT : themeStyle.muted }}><Highlighter className="w-4 h-4" /><span className="text-[9px] mt-1">형광펜</span></button>
        <button onClick={toggleBM} onContextMenu={e => { e.preventDefault(); setShowBookmarkPanel(!showBookmarkPanel) }} className="flex flex-col items-center justify-center py-2 rounded-lg hover:opacity-70" style={{ color: isCurrentPageBookmarked ? ACCENT : themeStyle.muted }}>{isCurrentPageBookmarked ? <BookmarkCheck className="w-4 h-4" /> : <Bookmark className="w-4 h-4" />}<span className="text-[9px] mt-1">책갈피</span></button>
        <button onClick={() => setShowSettings(!showSettings)} className="flex flex-col items-center justify-center py-2 rounded-lg hover:opacity-70" style={{ color: showSettings ? ACCENT : themeStyle.muted }}><Settings2 className="w-4 h-4" /><span className="text-[9px] mt-1">설정</span></button>
      </div>
      </div>
