...
.epub-focus-active .epub-content [data-epub-adjacent="true"]{opacity:0.25!important}
  ...
  // 선택 오버레이
  useEffect(() => {
    const ov = selectionOverlayRef.current; const ct = paginationContainerRef.current; if (!ov || !ct) return
    const update = () => { const s = window.getSelection(); if (!s || s.isCollapsed || !s.toString().trim()) { ov.innerHTML = ''; return }; try { const r = s.getRangeAt(0); const rects = r.getClientRects(); const cr = ct.getBoundingClientRect(); let h = ''; for (let i = 0; i < rects.length; i++) { const rc = rects[i]; if (rc.width < 1 || rc.right < cr.left || rc.left > cr.right) continue; h += `<div style="position:absolute;left:${Math.max(0,rc.left-cr.left)}px;top:${rc.top-cr.top}px;width:${Math.min(rc.right,cr.right)-Math.max(rc.left,cr.left)}px;height:${rc.height}px;background:rgba(245,158,11,0.35);pointer-events:none;border-radius:2px"></div>` }; ov.innerHTML = h } catch { ov.innerHTML = '' } }
    const onSel = () => requestAnimationFrame(update); const onUp = () => { setTimeout(() => { const s = window.getSelection(); if (!s || s.isCollapsed) ov.innerHTML = '' }, 100) }
    document.addEventListener('selectionchange', onSel); document.addEventListener('mouseup', onUp)
    return () => { document.removeEventListener('selectionchange', onSel); document.removeEventListener('mouseup', onUp) }
  }, [])

  // 글로벌 스타일
  useEffect(() => { const id = 'epub-lite-styles'; let el = document.getElementById(id) as HTMLStyleElement | null; if (!el) { el = document.createElement('style'); el.id = id; document.head.appendChild(el) }; el.textContent = `.epub-content::selection,.epub-content *::selection{background-color:rgba(245,158,11,0.4)!important}.epub-content mark[data-hl-color="yellow"]{background-color:rgba(250,220,50,0.3)!important}.epub-content mark[data-hl-color="green"]{background-color:rgba(100,220,100,0.25)!important}.epub-content mark[data-hl-color="blue"]{background-color:rgba(90,180,250,0.25)!important}.epub-content mark[data-hl-color="pink"]{background-color:rgba(245,130,180,0.3)!important}`; return () => { el?.remove() } }, [themeStyle.text])
  ...
  return (
    <div className="h-full flex flex-col" style={{ backgroundColor: themeStyle.pageBg, fontFamily: "'Noto Sans KR', system-ui, sans-serif" }}>
      ...
      {/* 검색 패널 */}
      {showSearch && (<div className="fixed inset-0 z-[60] flex flex-col"><div className="absolute inset-0 bg-black/40" onClick={() => setShowSearch(false)} /><div className="relative mx-auto mt-2 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden" style={{ backgroundColor: themeStyle.bg, maxHeight: '70vh' }}><div className="flex items-center gap-2 px-4 py-3 border-b" style={{ borderColor: themeStyle.border }}><Search className="w-4 h-4 flex-shrink-0" style={{ color: themeStyle.muted }} /><input ref={searchInputRef} type="text" value={searchQuery} onChange={e => { setSearchQuery(e.target.value); doSearch(e.target.value) }} onKeyDown={e => { if (e.key === 'Escape') setShowSearch(false) }} placeholder="본문 검색..." className="flex-1 bg-transparent outline-none text-sm" style={{ color: themeStyle.text }} /><button onClick={() => setShowSearch(false)} className="p-1 rounded hover:opacity-70" style={{ color: themeStyle.muted }}><X className="w-4 h-4" /></button></div><div className="overflow-y-auto" style={{ maxHeight: 'calc(70vh - 56px)' }}>{searchQuery && searchResults.length === 0 && <p className="px-4 py-8 text-center text-sm" style={{ color: themeStyle.muted }}>검색 결과가 없습니다</p>}{searchResults.map((r, i) => (<button key={i} className="w-full text-left px-4 py-3 border-b hover:opacity-80" style={{ borderColor: themeStyle.border }} onClick={() => { goToChapter(r.chapterIdx); setShowSearch(false) }}><span className="text-[10px] font-medium block mb-1" style={{ color: ACCENT }}>{r.chapterTitle}</span><p className="text-xs leading-relaxed" style={{ color: themeStyle.text }} dangerouslySetInnerHTML={{ __html: r.snippet.replace(new RegExp(`(${searchQuery.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi'), `<span style="background:${ACCENT}44;border-radius:2px;padding:0 2px">$1</span>`) }} /></button>))}</div></div></div>)}

      {/* 상단 바 */}
      <div className="flex justify-center border-b" style={{ borderColor: themeStyle.border }}>
      <div className="grid grid-cols-7 px-1 py-2 w-full" style={{ maxWidth: 420 }}>
        <button onClick={() => setShowToc(!showToc)} className="flex flex-col items-center justify-center py-1 rounded-lg hover:opacity-70" style={{ color: showToc ? ACCENT : themeStyle.muted }}><List className="w-4 h-4" /><span className="text-[9px] mt-0.5">목차</span></button>
        <button onClick={() => setShowSearch(!showSearch)} className="flex flex-col items-center justify-center py-1 rounded-lg hover:opacity-70" style={{ color: showSearch ? ACCENT : themeStyle.muted }}><Search className="w-4 h-4" /><span className="text-[9px] mt-0.5">검색</span></button>
        <button onClick={() => setFocusMode(!focusMode)} className="flex flex-col items-center justify-center py-1 rounded-lg" style={{ color: focusMode ? ACCENT : themeStyle.muted, backgroundColor: focusMode ? `${ACCENT}15` : 'transparent' }}><Focus className="w-4 h-4" /><span className="text-[9px] mt-0.5">집중</span></button>
        ...
        <button onClick={() => setShowHighlightPanel(!showHighlightPanel)} className="flex flex-col items-center justify-center py-1 rounded-lg" style={{ color: showHighlightPanel ? ACCENT : highlights.length > 0 ? ACCENT : themeStyle.muted }}><Highlighter className="w-4 h-4" /><span className="text-[9px] mt-0.5">형광펜</span></button>
        <button onClick={toggleBM} onContextMenu={e => { e.preventDefault(); setShowBookmarkPanel(!showBookmarkPanel) }} className="flex flex-col items-center justify-center py-1 rounded-lg hover:opacity-70" style={{ color: isCurrentPageBookmarked ? ACCENT : themeStyle.muted }}>{isCurrentPageBookmarked ? <BookmarkCheck className="w-4 h-4" /> : <Bookmark className="w-4 h-4" />}<span className="text-[9px] mt-0.5">책갈피</span></button>
        <button onClick={() => setShowSettings(!showSettings)} className="flex flex-col items-center justify-center py-1 rounded-lg hover:opacity-70" style={{ color: showSettings ? ACCENT : themeStyle.muted }}><Settings2 className="w-4 h-4" /><span className="text-[9px] mt-0.5">설정</span></button>
      </div>
      </div>
